...
class Evaluator extends tree_1.AbstractParseTreeVisitor {
    ...
    constructor(templates, opt) {
        ...
        this.customizedEvaluatorLookup = (baseLookup) => (name) => {
            ...
            if (templates_1.Templates.enableFromFile) {
        ...
        for (const item of values) {
            if (templateExtensions_1.TemplateExtensions.isPureExpression(item)) {
                ...
                for (const child of item.children) {
                    if (child instanceof LGTemplateParser_1.ExpressionInStructureContext) {
                        ...
                        switch (node.symbol.type) {
                            case LGTemplateParser_1.LGTemplateParser.ESCAPE_CHARACTER_IN_STRUCTURE_BODY:
    ...
    visitNormalTemplateString(ctx) {
        ...
        for (const child of ctx.children) {
            if (child instanceof LGTemplateParser_1.ExpressionContext) {
                result.push(this.evalExpression(child.text, child, ctx.text, prefixErrorMsg));
            }
            else {
                const node = child;
                switch (node.symbol.type) {
                    case LGTemplateParser_1.LGTemplateParser.MULTILINE_SUFFIX:
                    case LGTemplateParser_1.LGTemplateParser.MULTILINE_PREFIX:
                    case LGTemplateParser_1.LGTemplateParser.DASH:
                        break;
                    case LGTemplateParser_1.LGTemplateParser.ESCAPE_CHARACTER:
