...
class QnACardBuilder {
    ...
    static getSuggestionsCard(suggestionsList, cardTitle, cardNoMatchText) {
        ...
        const promptsCard = botbuilder_core_1.CardFactory.heroCard(cardTitle, undefined, buttonList);
        const message = botbuilder_core_1.MessageFactory.attachment(promptsCard);
        return message;
    }
    /**
     ...
     */
    static getQnAAnswerCard(result, displayPreciseAnswerOnly) {
        var _a, _b;
        if (!result) {
            throw new Error('Missing QnAMaker result');
        }
        const chatActivity = { type: botbuilder_core_1.ActivityTypes.Message };
        const buttonList = [];
        (_b = (_a = result.context) === null || _a === void 0 ? void 0 : _a.prompts) === null || _b === void 0 ? void 0 : _b.forEach((prompt) => {
            buttonList.push({
                value: prompt.qnaId,
                type: 'messageBack',
        ...
        if (buttonList.length > 0 || heroCardText.length > 0) {
            ...
            const prompt = botbuilder_core_1.CardFactory.heroCard('', heroCardText, undefined, buttonList);
